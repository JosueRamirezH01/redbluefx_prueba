  ...
  class _CreateAlertScreenState extends ConsumerState<CreateAlertScreen> {
    ...
    Future<void> _openPreviewAlert() async {
      ...
      if ((_selectedBase == null && (_manualBase == null || _manualBase!.isEmpty)) ||
          (_selectedQuote == null && (_manualQuote == null || _manualQuote!.isEmpty))) {
        Fluttertoast.showToast(msg: 'Selecciona o escribe ambas divisas', backgroundColor: Colors.redAccent,toastLength: Toast.LENGTH_LONG, gravity: ToastGravity.BOTTOM);
        return;
      }
      ...
      final alertDraft = Alert(
        ...
        parOne: _selectedBase?.partwo ?? _manualBase,
        parTwo: _selectedQuote?.partwo ?? _manualQuote,
        type: _selectedType!,
        isPublic: _isPublic,
    ...
    void _updatePairController() {
      final base = _selectedBase?.parone ?? _manualBase;
      final quote = _selectedQuote?.parone ?? _manualQuote;

      // si el usuario borró todo → limpiar también
      ...
      if (base != null && quote != null) {
        _pairController.text = "$base/$quote";
      }
    }
